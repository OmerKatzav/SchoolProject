...
public class ServiceFactory<TService, TImplementation>(IServiceProvider serviceProvider) : LambdaServiceFactory<TService>(GetServiceFunc(serviceProvider)) where TImplementation : TService where TService : class
{
    private static Func<TService> GetServiceFunc(IServiceProvider serviceProvider)
    {
        ...
        var arguments = constructor.GetParameters().Select(p => Expression.Call(Expression.Constant(serviceProvider), getService.MakeGenericMethod(p.ParameterType)));
